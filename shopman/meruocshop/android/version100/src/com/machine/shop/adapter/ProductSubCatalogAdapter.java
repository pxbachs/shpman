...
public class ProductSubCatalogAdapter extends BaseAdapter {
	...
	public ProductSubCatalogAdapter(Context context, ArrayList<ProductCatalog> aProductCatalog) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (views[position] == null) {
			...
			if (views[position] == null) {
				gridView = inflater.inflate(R.layout.product_sub_cat_item, null);
			} else {
			...
			aq.id(R.id.product_sub_cat_img).progress(R.id.progress).image(avatar_url, false, true, 0, R.drawable.placeholder_category, present, AQuery.FADE_IN);
			aq.id(R.id.product_sub_cat_name).text(mProductCatalog[position].getName());

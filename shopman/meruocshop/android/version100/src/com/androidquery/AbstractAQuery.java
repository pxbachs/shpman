...
public abstract class AbstractAQuery<T extends AbstractAQuery<T>> implements Constants {
	...
	/**
	 * Apply the transformer to convert raw data to desired object type for the
	 * next ajax request.
	 * 
	 * @param transformer
	 *            transformer
	 ...
	 *            Use file cache.
	 * @param targetWidth
	 *            Target width for down sampling when reading large images. 0 =
	 *            no downsampling.
	 * @param fallbackId
	 *            Fallback image if result is network fetch and image convert
	 *            failed. 0 = no fallback.
	 * @return self
	 * 
	 * @see testImage6
	 */
	 ...
	 *            failed. 0 = no fallback.
	 * @param preset
	 *            Default image to show before real image loaded. null = no
	 *            preset.
	 * @param animId
	 *            Apply this animation when image is loaded. 0 = no animation.
	 *            Also accept AQuery.FADE_IN as a predefined 500ms fade in
	 *            animation.
	 * @return self
	 * 
	 * @see testImage7
	 ...
	 *            animation.
	 * @param ratio
	 *            The desired aspect ratio of the imageview. Ratio is height /
	 *            width, or AQuery.RATIO_PRESERVE to preserve the original
	 ...
	 *            Use file cache.
	 * @param targetWidth
	 *            Target width for down sampling when reading large images.
	 * @param resId
	 *            Fallback image if result is network fetch and image convert
	 *            failed.
	 * @param callback
	...
	public T touch(View.OnTouchListener listener) {
		...
		if (view instanceof AdapterView) {
			...
			AdapterView<?> alv = (AdapterView<?>) view;
			alv.setOnTouchListener(listener);
	...
	/**
	 * Advanced Ajax callback. User must manually prepare the callback object
	 * settings (url, type, etc...) by using its methods.
	 * 
	 * @param callback
	 *            callback handler
	 * @return self
	 ...
	 * 
	 * The expire param is the duration to consider cached data expired (if
	 * hit). For example, expire = 15 * 60 * 1000 means if the cache data is
	 * within 15 minutes old, return cached data immediately, otherwise go fetch
	 * the source again.
	 * 
	 * 
	 * @param url
	 ...
	 *            data type
	 * @param expire
	 *            duration in millseconds, 0 = always use cache
	 * @param callback
	 *            callback handler
	 * @return self
	 * 
	 ...
	 *            data type
	 * @param handler
	 *            the handler object with the callback method to be called
	 * @param callback
	 ...
	 * For example, expire = 15 * 60 * 1000 means if the cache data is within 15
	 * minutes old, return cached data immediately, otherwise go fetch the
	 * source again.
	 * 
	 * 
	 * @param url
	...
	/**
	 * Ajax call with that block until response is ready. This method cannot be
	 * called on UI thread.
	 * 
	 * 
	 * @param callback
	 ...
	 *            url to cache
	 * @param expire
	 *            duration in millseconds, 0 = never consider cached data as
	 *            expired
	 * 
	 * @return self
	 * 
	...
	/**
	 * Return bitmap cached by image requests. Returns null if url is not
	 * cached.
	 * 
	 * @param url
	 * @return Bitmap
	...
	/**
	 * Return cached bitmap with a resourceId. Returns null if url is not
	 * cached.
	 * 
	 * Use this method instead of BitmapFactory.decodeResource(getResources(),
	 * resId) for caching.
	 * 
	 * @param resId
	 * 
	...
	/**
	 * @deprecated As of release 0.21.3, replaced by shouldDelay(int position,
	 *             View convertView, ViewGroup parent, String url)
	 * 
	 *             This method is less efficient and promote file check which
	 *             could hold up the UI thread.
	 * 
	...
	/**
	 * Determines if a group item of an expandable list should delay loading a
	 * url resource.
	 * 
	 * Designed to be used inside getGroupView(int groupPosition, boolean
	 * isExpanded, View convertView, ViewGroup parent) of an expandable list
	 * adapter.
	 * 
	 * @param groupPosition
	 *            the group position of the item
	 * @param isExpanded
	 *            the group is expanded
	 * @param convertView
	 *            the list item view
	 * @param parent
	 *            the parent input of getView
	 * @param url
	 *            the content url to be checked if cached and is available
	 *            immediately
	 * 
	...
	/**
	 * Determines if a child item of an expandable list item should delay
	 * loading a url resource.
	 * 
	 * Designed to be used inside getChildView(int groupPosition, int
	 * childPosition, boolean isLastChild, View convertView, ViewGroup parent)
	 * of an expandable list adapter.
	 * 
	 * @param groupPosition
	 *            the group position of the item
	 * @param childPosition
	...
	/**
	 * Determines if a list or gallery view item should delay loading a url
	 * resource because the view is scrolling very fast. The primary purpose of
	 * this method is to skip loading remote resources (such as images) over the
	 * internet until the list stop flinging and the user is focusing on the
	 * displaying items.
	 * 
	 * If the scrolling stops and there are delayed items displaying, the
	 * getView method will be called again to force the delayed items to be
	 * redrawn. During redraw, this method will always return false, thus
	 * allowing a particular resource to be loaded.
	 * 
	 * Designed to be used inside getView(int position, View convertView,
	 * ViewGroup parent) of an adapter.
	 * 
	 * 
	 * <br>
	 * <br>
	 ...
	 * 
	 * <br>
	 * This method uses the setOnScrollListener() method and will override any
	 * previously non-aquery assigned scroll listener. If a scrolled listener is
	 * required, use the aquery method scrolled(OnScrollListener listener) to
	 * set the listener instead of directly calling setOnScrollListener().
	 * 
	 * @param position
	 ...
	 * 
	 * Zoom is enabled without zoom control. Default background color is solid
	 * black (0xFF000000).
	 * 
	 * @param url
	 *            The image url
	 ...
	 * Load the webview with an image with a url.
	 * 
	 * Zoom control can only be disabled for API level 14+. Lower level apis
	 * will always show zoom control if zoom is enabled.
	 * 
	 * Note that color is AARRGGBB, where AA is the alpha value. Solid color
	 * (0xFF??????) should be used for performance reason. Transparent
	 * background is not well supported by WebView in most devices.
	 * 
	 * @param url
	 *            The image url
	 * @param zoom
